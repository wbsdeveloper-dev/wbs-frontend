...
function mapContractToRow(
  ...
  const awalYear = contract.awal_perjanjian
    ? new Date(contract.awal_perjanjian).getFullYear()
    : null;
  const akhirYear = contract.akhir_perjanjian
    ? new Date(contract.akhir_perjanjian).getFullYear()
    : null;
  ...
  const row: ContractTableRow = {
    ...
    _mmscfd_volumeJPMH:
      contract.volume_jpmh_mmscfd != null
        ? String(contract.volume_jpmh_mmscfd)
        : "",
...
function buildColumns(
  ...
  const makeVolumeYearRenderCell = (
    year: number,
    options?: { suffix?: string; readOnly?: boolean; fractionDigits?: number },
  ) => {
          ...
          <Box
            sx={{
              ...
              backgroundColor: readOnly
                ? "var(--surface-muted)"
                : "var(--surface)",
              color: readOnly ? "var(--text-subtle)" : "var(--text-primary)",
              px: 1,
              mx: "auto",
              fontSize: "12px",
              overflow: "hidden",
  ...
  for (const year of years) {
    ...
    const topRenderCell = makeVolumeYearRenderCell(year, {
      readOnly: true,
      fractionDigits: 2,
    });
    const percentTopRenderCell = makeVolumeYearRenderCell(year, {
      suffix: "%",
    });
    cols.push(
      {
...
export default function ContractTable() {
  ...
  const [activeTab, setActiveTab] = useState<"Info Kontrak" | "TJK">(
    "Info Kontrak",
  );
  ...
  const [documentModalRowId, setDocumentModalRowId] = useState<string | null>(
    null,
  );
  ...
  const { data: powerplantSitesData } = useSites({
    type: "PEMBANGKIT",
    commodity: "GAS PIPA",
  });
  ...
  const powerplantSites = useMemo(
    () => powerplantSitesData || [],
    [powerplantSitesData],
  );
  const supplierNames = useMemo(
    () => supplierSites.map((s) => s.name),
    [supplierSites],
  );
  const powerplantNames = useMemo(
    () => powerplantSites.map((s) => s.name),
    [powerplantSites],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          const fallbackRows = contracts.map((c, i) =>
            mapContractToRow(c, i, [], [], [], null, yearRange),
          );
  ...
  const processRowUpdate = useCallback((newRow: ContractTableRow) => {
  ...
  const handleExport = useCallback(() => {
    ...
    const excelData = rows.map((row) => {
      ...
      for (const year of yearRange) {
        if (
          row._akhirPerjanjianYear !== null &&
          row._akhirPerjanjianYear !== undefined &&
          year > row._akhirPerjanjianYear
        ) {
          ...
          if (topValue !== null && topValue !== undefined && topValue !== "") {
            ...
            rowData[`Volume ${year} TOP`] = !isNaN(topNum)
              ? topNum.toFixed(2)
              : topValue;
          } else {
    ...
    XLSX.writeFile(
      workbook,
      `kontrak_gas_pipa_${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  }, [rows, yearRange]);
  ...
  const saveSubResources = useCallback(
    async (contractId: string, row: ContractTableRow) => {
      ...
      for (const year of yearRange) {
        // JPH
        if (
          bVal(`volume${year}JPH`) !== undefined ||
          mVal(`volume${year}JPH`) !== undefined
        ) {
          volumeItems.push({
            year,
            basis: "JPH",
        ...
        if (
          bVal(`volume${year}TOP`) !== undefined ||
          bVal(`volume${year}PercentTOP`) !== undefined ||
          mVal(`volume${year}TOP`) !== undefined ||
          mVal(`volume${year}PercentTOP`) !== undefined
        ) {
          volumeItems.push({
            year,
            basis: "TOP",
        ...
        // Kepmen
        if (
          bVal(`volumeKepmen${year}`) !== undefined ||
          mVal(`volumeKepmen${year}`) !== undefined
        ) {
          volumeItems.push({
            year,
            basis: "JPH",
  ...
  const handleSave = useCallback(async () => {
    ...
    const currentRows = await new Promise<ContractTableRow[]>((resolve) => {
      setRows((prev) => {
        resolve(prev);
        return prev;
      });
    ...
    try {
      ...
      for (const row of currentRows) {
        if (row._isNew) {
          ...
          if (!partyId) {
            const selectedSupplier = supplierSites.find(
              (s) => s.name === row.pemasok,
            );
            const selectedPowerplant = powerplantSites.find(
              (s) => s.name === row.pembangkit,
            );
          ...
          const newJpmhPrefix =
            row.unitSwitch === "BBTUD" ? "_bbtud_" : "_mmscfd_";
          row[`${newJpmhPrefix}volumeJPMH`] = row.volumeJPMH;

          if (row._bbtud_volumeJPMH)
            createPayload.volume_jpmh_bbtud = parseFloat(
              String(row._bbtud_volumeJPMH),
            );
          if (row._mmscfd_volumeJPMH)
            createPayload.volume_jpmh_mmscfd = parseFloat(
              String(row._mmscfd_volumeJPMH),
            );
          if (row._bbtud_TJK)
            createPayload.tjk_bbtud = parseFloat(String(row._bbtud_TJK));
          if (row._mmscfd_TJK)
            createPayload.tjk_mmscfd = parseFloat(String(row._mmscfd_TJK));
          if (row.hargaPJBG) createPayload.price_value = row.hargaPJBG;
          if (row.hgbt) createPayload.hgbt_value = parseFloat(row.hgbt);

          const createdContract = await createMutation.mutateAsync(
            createPayload as unknown as CreateContractPayload,
          );
          ...
          if (newPrice !== original.price_value) payload.price_value = newPrice;
          ...
          const newVolBbtud = row._bbtud_volumeJPMH
            ? parseFloat(row._bbtud_volumeJPMH)
            : null;
          if (newVolBbtud !== original.volume_jpmh_bbtud)
            payload.volume_jpmh_bbtud = newVolBbtud;

          const newVolMmscfd = row._mmscfd_volumeJPMH
            ? parseFloat(row._mmscfd_volumeJPMH)
            : null;
          if (newVolMmscfd !== original.volume_jpmh_mmscfd)
            payload.volume_jpmh_mmscfd = newVolMmscfd;

          const newTjkBbtud = row._bbtud_TJK
            ? parseFloat(row._bbtud_TJK)
            : null;
          if (newTjkBbtud !== original.tjk_bbtud)
            payload.tjk_bbtud = newTjkBbtud;

          const newTjkMmscfd = row._mmscfd_TJK
            ? parseFloat(row._mmscfd_TJK)
            : null;
          ...
          if (row.pemasok !== ((original as any).pemasok_name || "")) {
            const newSupplier = supplierSites.find(
              (s) => s.name === row.pemasok,
            );
            partyPayload.pemasok_site_id = newSupplier ? newSupplier.id : null;
          }
          if (row.pembangkit !== ((original as any).pembangkit_name || "")) {
            const newPowerplant = powerplantSites.find(
              (s) => s.name === row.pembangkit,
            );
            partyPayload.pembangkit_site_id = newPowerplant
              ? newPowerplant.id
              : null;
          }
          ...
          >
            <IconButton
              ...
              sx={{
                ...
                "&:hover": {
                  color: document ? "var(--theme-primary)" : "#d97706",
                },
              }}
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-primary bg-primary/5 border border-primary/20 rounded-lg hover:bg-primary/10 transition-all duration-200"
          >
            <FileText size={16} />
            Ekspor
          </button>
                  ...
                  <Box sx={{ px: 2, pb: 2, pt: 1 }}>
                    <DataGrid
                      ...
                      isCellEditable={(params) => {
                        ...
                        const yearMatch = field.match(
                          /^(?:volume(\d{4})(?:JPH|TOP|PercentTOP)|jumlahKontrakTahunan(\d{4})|volumeKepmen(\d{4}))$/,
                        );
                        if (yearMatch) {
                          const year = parseInt(
                      ...
                      onCellClick={(params) => {
                        if (isEditMode && params.isEditable) {
                          ...
                          const isEditing =
                            state?.editRows?.[params.id]?.[params.field];
                          if (!isEditing) {
                        ...
                        "& .MuiDataGrid-row": {
                          "&:hover": {
                            backgroundColor: isEditMode
                              ? "color-mix(in srgb, var(--surface) 86%, #f59e0b 14%)"
                              : "var(--surface-muted)",
                          },
                        },
        ...
        <DialogContent dividers>
          {(() => {
            ...
            const isUploadingThis =
              uploadMutation.isPending && uploadingRowId === documentModalRowId;
                        ...
                        <IconButton
                          ...
                          onClick={async () => {
                            try {
                              ...
                              setSnackbar({
                                open: true,
                                message: "Gagal memuat dokumen untuk pratinjau",
                                severity: "error",
                              });
                            }
                          }}
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button
            onClick={async () => {
              if (selectedFile && documentModalRowId) {
                try {
                  ...
                  const documents = await getContractDocuments(
                    documentModalRowId,
                  ).catch(() => []);
                  ...
                  setRows((prev) =>
                    prev.map((r) =>
                      r.id === documentModalRowId
                        ? { ...r, document: latestDocument }
                        : r,
                    ),
                  );
            ...
            sx={{
              ...
              borderRadius: "8px",
              backgroundColor: "var(--theme-primary)",
              "&:hover": { backgroundColor: "var(--theme-primary-dark)" },
              "&.Mui-disabled": {

...
export default function EmailInboxTable() {
  ...
  const [emailToDelete, setEmailToDelete] = useState<EmailInboxRecord | null>(
    null,
  );
  ...
  const handleToggleSelectAll = () => {
    if (
      selectedIds.length === paginatedInbox.length &&
  ...
  const handleConfirmDeleteBulk = async () => {
    ...
    try {
      ...
      alert(
        err instanceof Error ? err.message : "Gagal menghapus email terpilih",
      );
    } finally {
      setIsDeleting(false);
    }
        ...
        <div className="flex items-center gap-2">
          {canRead && (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border transition-all duration-200 cursor-pointer ${showFilters || activeFilterCount > 0
        ...
        {filteredInbox.length === 0 ? (
          <div className="text-center p-8 bg-white rounded-lg border border-gray-200">
            <p className="text-sm text-gray-500">
              Tidak ada data email yang cocok dengan filter yang dipilih.
            </p>
            <button
              onClick={handleResetFilters}
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-4 py-3 text-xs font-bold text-gray-900 uppercase tracking-wider w-[180px]">
                      Waktu Diterima
                    </th>
                    <th className="px-4 py-3 text-xs font-bold text-gray-900 uppercase tracking-wider">
                      Nama Rule
                    </th>
                    <th className="px-4 py-3 text-xs font-bold text-gray-900 uppercase tracking-wider">
                      Pengirim
                    </th>
                    <th className="px-4 py-3 text-xs font-bold text-gray-900 uppercase tracking-wider min-w-[220px]">
                      Subjek
                    </th>
                    <th className="px-4 py-3 text-xs font-bold text-gray-900 uppercase tracking-wider text-center w-[130px]">
                      Status Diproses
                    </th>
                    <th className="px-4 py-3 text-xs font-bold text-gray-900 uppercase tracking-wider text-right w-[150px]">
                      Attachment
                    </th>
                    {canDelete && (
                      <th className="px-4 py-3 text-xs font-bold text-gray-900 uppercase tracking-wider text-center w-[90px]">
                        Aksi
                      </th>
                    )}
                  </tr>
                ...
                <tbody className="divide-y divide-gray-200">
                  {paginatedInbox.map((email: EmailInboxRecord) => (
                    <tr
                      key={email.id}
                      className={`hover:bg-gray-50 transition-colors ${selectedIds.includes(email.id) ? "bg-red-50/30" : ""
                        }`}
                    >
                      {canDelete && (
                      ...
                      <td className="px-4 py-3 text-right">
                        {email.attachment_refs &&
                          email.attachment_refs.length > 0 ? (
                          <div className="flex flex-col items-end gap-1.5">
                            {email.attachment_refs.map((att, idx) => {
                              const isDownloading =
                                downloadingRef === att.storageRef;
                              const isPreviewing =
                                previewingRef === att.storageRef;
                              return (
                                <div
                                  key={idx}
                                  className="flex items-center gap-1.5 justify-end w-full max-w-[280px]"
                                >
                                  <span
                    ...
                    return pages.map((p, idx) =>
                      p === "..." ? (
                        ...
                        <button
                          ...
                          className={`min-w-[2rem] h-8 rounded-lg text-sm font-medium transition-all duration-200 cursor-pointer ${p === safeCurrentPage
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6 space-y-3">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-xs space-y-1 font-mono text-gray-700">
                <div>
                  <span className="font-semibold text-gray-500">Pengirim:</span>{" "}
                  {emailToDelete.sender}
                </div>
                <div>
                  <span className="font-semibold text-gray-500">Subjek:</span>{" "}
                  {emailToDelete.subject || "(Tanpa Subjek)"}
                </div>
              </div>
            </div>
